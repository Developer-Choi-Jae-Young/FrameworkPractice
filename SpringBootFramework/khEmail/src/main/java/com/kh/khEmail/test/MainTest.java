...
public class MainTest {
	...
	public static void main(String[] args) throws MessagingException, UnsupportedEncodingException{
		...
		sender.setUsername("jy951213.choi@gmail.com");				// - 인증받은 사용자 이메일
		sender.setPassword("ksws ucbx dmzo erry"); // - 앱 비밀번호
		
		Properties prop = new Properties();
		prop.put("mail.smtp.auth", true);					// SMTP 서버 연결 시 사용자 인증 설정
		prop.put("mail.smtp.starttls.enable", true);		// SMTP 서버 연결 시 TLS 암호화 설정
		...
		helper.setFrom("jy951213.choi@gmail.com", "[KH] 메일 전송");		// 발신자 (이메일, 별칭)
		helper.setTo("cjy951213@naver.com");							// 수신자
		
		// 파일 첨부 (첨부파일)
		File f = new File(MainTest.class.getResource("/static/img/TEST.jpg").getPath());
